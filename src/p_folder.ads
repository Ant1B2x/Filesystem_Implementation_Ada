...
package P_Folder is
   ...
   -- Role : Check if the two given folders are the same
   -- Parameters :
   --    folder1 (T_Folder) : First folder to be compared with the second
   --    folder2 (T_Fodler) : Second folder to be compared with the first
   -- Return :
   --    Boolean : Return True if the two folders are the same, or False if not
   -- Preconditions : /
   -- Postconditions : /
